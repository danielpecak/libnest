...
def energy_per_nucleon(rho_n, rho_p):
    ...
    return (3*units.HBARC**2./20.*kF**2*((np.power(1+eta,5./3.)/
                                                   units.MN
                                                   +np.power(1-eta,5./3.)
                                                   /units.MP))
            + units.T0/8.*rho*(3-(2*units.X0 + 1)*eta**2)
            + 3*units.T1/40*rho*kF**2*((2+units.X1)*F_x_5
                                               -(1./2.+units.X1)*F_x_8)
            + 3./40*(units.T2X2*F_x_5
                                     -units.T2X2
                                             *F_x_8)*rho*kF**2
            + units.T3/48*np.power(rho,units.ALPHA+1)*
                (3-(1+2.*units.X3)*eta**2)
            + 3*units.T4/40.*kF**2*np.power(rho, units.BETA+1)
                *((2+units.X4)*F_x_5-(1./2.+units.X4)*F_x_8)
            + 3*units.T5/40*kF*np.power(rho,units.GAMMA+1)
                *((2+units.X5)*F_x_5-(1./2+units.X5)*F_x_8))
